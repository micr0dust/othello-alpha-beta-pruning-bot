...
╚═╝  ╚═╝╚══════╝╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝      ╚═════╝ ╚══════╝   ╚═╝   ╚═╝  ╚═╝
                ...
                ╚██████╔╝   ██║   ██║  ██║███████╗███████╗███████╗╚██████╔╝
                 ╚═════╝    ╚═╝   ╚═╝  ╚═╝╚══════╝╚══════╝╚══════╝ ╚═════╝ 
                                        made by Microdust (true stable version)
*/
...
extern "C"
{
    ...
    public:
        ...
        int true_stable_stones(Game& bitboard, uint64_t stable_map, int color) {
            uint64_t corners = CORNER_MASK;
            stable_map |= (bitboard.white & corners) | (bitboard.black & corners);

            bool updated = true;
            while (updated) {
                updated = false;
                for (int idx = 0; idx < NN; ++idx) {
                    uint64_t pos = 1ULL << STABLE_ORDER[idx];
                    if ((bitboard.white & pos) == 0 && (bitboard.black & pos) == 0) continue; // 跳過空格
                    if (stable_map & pos) continue; // 已經是穩定子

                    int team = (bitboard.white & pos) ? 1 : -1;
                    bool is_stable = true;

                    for (int dir : {LEFT, UP, UP_LEFT, DOWN_LEFT}) {
                        uint64_t neighbor = shift(pos, dir);
                        uint64_t opposite = shift(pos, -dir);

                        if ((neighbor & (LEFT_EDGE | RIGHT_EDGE | TOP_EDGE | BOTTOM_EDGE)) ||
                            (opposite & (LEFT_EDGE | RIGHT_EDGE | TOP_EDGE | BOTTOM_EDGE)) ||
                            (stable_map & neighbor & (team == 1 ? bitboard.white : bitboard.black)) ||
                            (stable_map & opposite & (team == 1 ? bitboard.white : bitboard.black)) ||
                            ((stable_map & neighbor & (team == -1 ? bitboard.white : bitboard.black)) &&
                            (stable_map & opposite & (team == -1 ? bitboard.white : bitboard.black)))) {
                            continue;
                        }
            ...
            int stable_count = 0;
            uint64_t color_map = (color == 1) ? bitboard.white : bitboard.black;
            for (int i = 0; i < NN; ++i) {
                if (stable_map & (1ULL << i) & color_map) {
                    stable_count++;
                }
            }

            stable_count += __builtin_popcountll(stable_map & corners & color_map) * CORNOR;
            return stable_count;
        }

        double evaluate(Game& bitboard, uint64_t stable_map, int color)
        {
            int actions = __builtin_popcountll(getValidMoves(bitboard, color));
            return (actions + true_stable_stones(bitboard, stable_map, color)) / static_cast<double>(MAX_SCORE);
        }
        ...
        double max_value(Game bitboard, uint64_t stable_map, int color, double alpha, double beta, int depth)
        {
            if (isEndGame(bitboard))
                return endgame_evaluation(bitboard, color);
            else if (depth == 0)
                return evaluate(bitboard, stable_map, color);
            ...
            for (auto position : valids)
            {
                ...
                double score = min_value(bitboard_copy, stable_map, -color, alpha, beta, depth - 1);
        ...
        double min_value(Game bitboard, uint64_t stable_map, int color, double alpha, double beta, int depth)
        {
            if (isEndGame(bitboard))
                return endgame_evaluation(bitboard, -color);
            else if (depth == 0)
            ...
            for (auto position : valids)
            {
                Game bitboard_copy = bitboard;
                executeMove(bitboard_copy, color, position);
                double score = max_value(bitboard_copy, stable_map, -color, alpha, beta, depth - 1);
            ...
            for (uint64_t position : valids)
            {
                futures.push_back(
                    ...
                    [this, bitboard, color, position, depth](){
                        ...
                        double score = min_value(bitboard_copy, 0, -color, alpha, beta, depth - 1);
